...
func TestNewStrategy(t *testing.T) {
	s := NewStrategy(nil, nil, nil, nil, nil, nil, nil)
	assert.NotNil(t, s)
	assert.Len(t, s.files, 0)

	s = NewStrategy(nil, []string{}, []string{}, []string{}, []string{}, nil, nil)
	assert.NotNil(t, s)
	assert.Len(t, s.files, 0)

	s = NewStrategy(nil, []string{"file"}, []string{"file"}, []string{}, []string{}, nil, nil)
	assert.NotNil(t, s)
	assert.Len(t, s.files, 1)

	s = NewStrategy(nil, []string{"file-1", "file-2"}, []string{"file-1", "file-2"}, []string{}, []string{}, nil, nil)
	assert.NotNil(t, s)
	...
	s = NewStrategy(conf, testFiles, testFiles, []string{}, []string{}, finder, ctx)
	assert.NotNil(t, s)
	assert.Len(t, s.files, 1)
...
func TestInvokeManyFilesWCorrectFilters(t *testing.T) {
	...
	ctx, _ := ctxTestdata.NewContextMock()
	s := NewStrategy(conf, testFiles, testFiles, []string{"test"}, []string{}, finder, ctx)
	jobs, _ := s.Invoke()
	assert.Len(t, jobs, 1)
