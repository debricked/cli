...
func TestMakeErr(t *testing.T) {
	...
	s, err := f.Make(conf, nil, nil, nil, nil, nil, nil)
	assert.Nil(t, s)
...
func TestMake(t *testing.T) {
	...
	cases := map[string]IStrategy{
		java.Name: java.NewStrategy(conf, []string{}, []string{}, []string{}, []string{}, nil, nil),
	}
	f := NewStrategyFactory()
	for name, strategy := range cases {
		t.Run(name, func(t *testing.T) {
			s, err := f.Make(conf, []string{}, []string{}, []string{}, []string{}, nil, nil)
