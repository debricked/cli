...
func TestNodeToBytes(t *testing.T) {
	...
	assert.Equal(t, "['symbol', False, False, 'Node', 'file.go', 1, 10, []]", string(bytes))

	node.IsLibraryNode = true
	bytes = node.ToBytes()
	assert.NotNil(t, bytes)
...
func TestNodeCount(t *testing.T) {
	cg := NewCallGraph()
	cg.AddNode("file.go", "Node1", "symbol1", false, 1, 10)
	cg.AddNode("file.go", "Node2", "symbol2", false, 11, 20)
...
func TestEdgeCount(t *testing.T) {
	cg := NewCallGraph()
	parent := cg.AddNode("file.go", "Parent", "symbol1", false, 1, 10)
	child1 := cg.AddNode("file.go", "Child1", "symbol2", false, 11, 20)
	child2 := cg.AddNode("file.go", "Child2", "symbol3", false, 21, 30)
