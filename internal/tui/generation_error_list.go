...
type CallgraphJobsErrorList struct {
	mirror io.Writer
	jobs   []job.IJob
}

func NewCallgraphJobsErrorList(mirror io.Writer, jobs []job.IJob) CallgraphJobsErrorList {
	return CallgraphJobsErrorList{mirror: mirror, jobs: jobs}
}

func (jobsErrList CallgraphJobsErrorList) Render() error {
...
func (jobsErrList CallgraphJobsErrorList) addJob(list *bytes.Buffer, job job.IJob) {
	var jobString string
	if !job.Errors().HasError() {
		return
	}

	list.Write([]byte(fmt.Sprintf("%s\n", color.YellowString(job.GetDir()))))
...
func (jobsErrList CallgraphJobsErrorList) createErrorString(err error, warning bool) string {
	var pipe string
	if warning {
